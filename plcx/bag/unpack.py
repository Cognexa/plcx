...
def bytes_to_list(
    msg: bytes, format_: str, byte_order: str = BYTE_ORDER
) -> List[VALUE]:
    """
    ...
    for c, count in arguments_count:
        if c == BOOLEAN_FORMAT_SYMBOL:
            ...
            bytes_list.append(
                boolean_list[0] if len(boolean_list) == 1 else boolean_list
            )
...
def bytes_to_dict(
    ...
    values = bytes_to_list(
        msg=msg, format_="".join([f for _, f in config]), byte_order=byte_order
    )
